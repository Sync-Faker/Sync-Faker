    ...
    def __getitem__(self, index):
        ...
        if self.pix_fusion:
            ...
            for idx, channel in enumerate(self.CAM_CHANNELS):
                ...
                # ###############
                # #修改相机，滞后0.2s
                # cam_token_falsetime = cam_token
                # cam_channel_falsetime = cam_channel
                # if lidar_channel['next'] != '':
                #     for i in range(2):
                #         cam_token_falsetime = cam_channel_falsetime['next']
                #         cam_channel_falsetime = self.nusc.get('sample_data', cam_token_falsetime)
                # #############
        ...
        if self.version != "v1.0-test":
            lidarseg_labels_filename = os.path.join("/mnt/localDisk2/ssk/lcps-main/semantic_val_preds_falsetime/lidarseg/val",
                                                    self.nusc.get('lidarseg', lidar_sd_token)['filename'][23:])
            panoptic_labels_filename = os.path.join("/mnt/localDisk2/ssk/lcps-main/panoptic_val_preds_falsetime/panoptic/val",
                                                    self.nusc.get('panoptic', lidar_sd_token)['filename'][23:])
